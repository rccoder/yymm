...
! function(e) {
	...
	function n(e, t, i) {
		if (null == i && (null == t ? (i = e, e = null) : (i = t, t = null, e instanceof Array && (t = e, e = null))), null != i) {
			...
			e ? (o(e, t, i), D && clearTimeout(D)) : B[0] = {
				deps: t,
				factory: i
			}
		}
		...
		if (t && !d(e, J)) {
			...
			T(i, function(i, n) {
				...
				h && !H[h] ? (l.res && (s = {
					...
					absId: h,
					hard: a > n
				}, t.depMs.push(r), t.depMkv[h] = r, o.push(h))) : r = {
					absId: h
				}, a > n && t.factoryDeps.push(s || r)
			}), t.state = J, h(e), y(o)
		}
	}
				...
				if (T(a.factoryDeps, function(e) {
					...
					try {
						var r = c(o, {
							...
							s = a.factory,
							l = "function" == typeof s ? s.apply(e, r) : s;
		...
		if (e) {
			for (var i in G) {
				var n = e[i],
					a = G[i];
				if (n)
					if ("urlArgs" === i && "string" == typeof n) G.urlArgs["*"] = n;
					else if (a instanceof Array) T(n, t);
	...
	var Y, Q, q, Z, j, $, et, tt = document.getElementsByTagName("head")[0],
		it = document.getElementsByTagName("base")[0];
	...
	s.version = "2.1.10", s.dependencies = {
		zrender: "2.0.6"
	}, s.init = function(t, n) {
		var a = e("zrender");
	...
	return i.prototype = {
		...
		addData: function(e, t, i, n, r) {
			for (var s = e instanceof Array ? e : [
				[e, t, i, n, r]
			], l = this.getOption(), h = this._optionRestore, d = 0, m = s.length; m > d; d++) {
				...
				if (c) {
					...
					if (V[p](t), U[p](t), n || (V[u](), t = U[u]()), null != r) {
						...
						if (c.type === a.CHART_TYPE_PIE && (y = h.legend) && (g = y.data)) {
							var f = l.legend.data;
							if (g[p](r), f[p](r), !n) {
								var b = o.indexOf(g, t.name); - 1 != b && g.splice(b, 1), b = o.indexOf(f, t.name), -1 != b && f.splice(b, 1)
							}
	...
	return m.prototype.getId = function() {
		...
		if ("string" == typeof e && (e = this.storage.get(e)), e) {
			...
			if (t) {
				...
				s && (o = s)
			} else o = e; if (!o) return void n('Property "' + t + '" is not existed in element ' + e.id);
			var d = this.animatingElements;
			return "undefined" == typeof e.__aniCount && (e.__aniCount = 0), 0 === e.__aniCount && d.push(e), e.__aniCount++, this.animation.animate(o, {
				loop: a
			}).done(function() {
	...
	function t(e, t, a, o, s) {
		...
		h.shapeHandler.onmousewheel = function(e) {
			...
			var a = r.get(t, "value"),
				o = a * h.option.island.calculateStep;
	...
	return t.prototype = {
		...
		_combine: function(t, i) {
			var n = e("zrender/tool/color"),
				a = e("../util/accMath"),
				o = a.accAdd(r.get(t, "value"), r.get(i, "value")),
				s = r.get(t, "name") + this._nameConnector + r.get(i, "name");
		...
		add: function(e) {
			var t = r.get(e, "name"),
				i = r.get(e, "value"),
				n = null != r.get(e, "series") ? r.get(e, "series").name : "",
			...
			"#fff" === s.style.color && (s.style.color = e.style.strokeColor), this.setCalculable(s), s.dragEnableTime = 0, r.pack(s, {
				name: n
			}, -1, i, -1, t), s = new a(s), this.shapeList.push(s), this.zr.addShape(s)
		},
		del: function(e) {
	...
	return t.prototype = {
		...
		getMagicOption: function() {
			...
			if (this._defaultMagic(s.CHART_TYPE_CHORD, s.CHART_TYPE_FORCE), this._defaultMagic(s.CHART_TYPE_PIE, s.CHART_TYPE_FUNNEL), this._magicType[m] || this._magicType[c])
				for (var n = 0, a = this.option.series.length; a > n; n++) this._magicType[m] ? (this.option.series[n].stack = "_ECHARTS_STACK_KENER_2014_", t = m) : this._magicType[c] && (this.option.series[n].stack = null, t = c), this._featureOption[t + "Chart"] && l.merge(this.option.series[n], this._featureOption[t + "Chart"] || {}, !0);
			return this.option
		},
	...
	return t.prototype = {
		...
		_showPolarTrigger: function(e, t) {
			...
			if (r.length > 0) {
				...
				if (this._lastDataIndex != t || this._lastSeriesIndex != s[0])
							...
							switch (d.type) {
								...
								case s.CHART_TYPE_SCATTER:
									for (var c = e.dataIndex, a = 0, o = f.length; o > a; a++)
										if (l.get(f[a], "seriesIndex") == t && l.get(f[a], "dataIndex") == c) {
		...
		_getChainPoint: function() {
			...
			if (p > 1) {
				...
				if (V = V > 50 ? 50 : 20 > V ? 5 : V, c -= 2 * V, "number" === i.type)
					for (var U = 0; p > U; U++) u.push(d + V + c / (p - 1) * U);
				else {
	...
	return n.inherits(t, i), t.prototype._start = function(e, t) {
		...
		for (var h = this.createTextShape(i.textStyle), d = this.createBackgroundShape(i.backgroundColor), m = s.x, c = s.y, p = s.r0 + 6, u = s.r - 6, V = s.color, U = a.lift(V, .1), y = new o({
			highlightStyle: n.clone(s)
		}), g = [], f = a.getGradientColors(["#ff6400", "#ffe100", "#97ff00"], 25), b = 15, _ = 240, x = 0; 16 > x; x++) g.push(new r({
			highlightStyle: {
				x: m,
				y: c,
				r0: p,
			...
			_color: a.getLinearGradient(m + p * Math.cos(_, !0), c - p * Math.sin(_, !0), m + p * Math.cos(_ - b, !0), c - p * Math.sin(_ - b, !0), [
				[0, f[2 * x]],
				[1, f[2 * x + 1]]
			])
		})), _ -= b;
	...
	return document.createElement("canvas").getContext ? G_vmlCanvasManager = !1 : ! function() {
		...
		for (var D = [], N = 0; 16 > N; N++)
			for (var H = 0; 16 > H; H++) D[16 * N + H] = N.toString(16) + H.toString(16);
		var B = {
				aliceblue: "#F0F8FF",
				antiquewhite: "#FAEBD7",
				aquamarine: "#7FFFD4",
		...
		q.clearRect = function() {
			...
			var m = e.width,
				c = e.height;
			if (e.runtimeStyle.width = h, e.runtimeStyle.height = d, 3 == arguments.length) t = arguments[1], i = arguments[2], o = r = 0, s = n = m, l = a = c;
			else if (5 == arguments.length) t = arguments[1], i = arguments[2], n = arguments[3], a = arguments[4], o = r = 0, s = m, l = c;
			else {
		...
		u = {
			...
			mousemove: function(e) {
				if (!this.painter.isLoading()) {
					...
					var n = "default";
					if (this._draggingTarget) this.storage.drift(this._draggingTarget.id, t, i), this._draggingTarget.modSelf(), this.storage.addHover(this._draggingTarget);
					else if (this._isMouseDown) {
						var a = this.painter.getLayers(),
							o = !1;
						for (var r in a)
							if ("hover" !== r) {
								var l = a[r];
	...
	V.prototype.on = function(e, t) {
		...
		e ? o.cancelBubble || this.dispatch(t, o) : n || this.dispatch(t, {
			type: t,
			event: i
		})
	...
	var a = e("./config"),
		...
		r = e("./tool/log"),
		s = e("./tool/matrix"),
		l = e("./loadingEffect/Base"),
		h = e("./mixin/Transformable"),
		d = window.devicePixelRatio || 1;
	d = Math.max(d, 1);
	var m = window.G_vmlCanvasManager,
		c = function(e, i) {
	...
	c.prototype.render = function(e) {
		...
		for (var h = [], d = 0, c = e.length; c > d; d++) {
			...
			if ((i.dirty || t) && !p.invisible && (!p.onbrush || p.onbrush && !p.onbrush(o, !1)))
		...
		this.storage.iterShape(function(e) {
			if (!e.invisible && (!e.onbrush || e.onbrush && !e.onbrush(s, !1)))
				if (a.catchBrushException) try {
	...
	function r(e, t, i, n, a, o, r, l, h) {
		var d = e.length;
		if (1 == h)
			for (var m = 0; d > m; m++) l[m] = s(e[m], t[m], i[m], n[m], a, o, r);
		else
			for (var c = e[0].length, m = 0; d > m; m++)
				for (var p = 0; c > p; p++) l[m][p] = s(e[m][p], t[m][p], i[m][p], n[m][p], a, o, r)
	}
					...
					if (U) {
						...
						if (U) {
							...
							var A = function(e, o) {
									...
									if (0 !== d) {
										if (w = (o - x[L]) / d, c)
											if (I = k[L], X = k[0 === L ? L : L - 1], K = k[L > U - 2 ? U - 1 : L + 1], S = k[L > U - 3 ? U - 1 : L + 2], g) r(X, I, K, S, w, w * w, w * w * w, l(e, V), b);
											else {
												var m;
												f ? (m = r(X, I, K, S, w, w * w, w * w * w, E, 1), m = h(E)) : m = s(X, I, K, S, w, w * w, w * w * w), i(e, V, m)
											} else if (g) a(k[L], k[L + 1], w, l(e, V), b);
										else {
											var m;
		...
		if (t(m) && t(c))
			...
			if (t(g)) {
				...
				var L = (2 * m * l - 3 * s * c) / (2 * Math.sqrt(m * m * m)),
					...
					w = Math.cos(v),
					V = (-l - 2 * W * w) / (3 * s),
					b = (-l + W * (w + U * Math.sin(v))) / (3 * s),
					X = (-l + W * (w - U * Math.sin(v))) / (3 * s);
	...
	function t(e, t, a, o, r) {
		...
		s._onlegendhoverlink = function(e) {
			if (s.legendHoverLink)
				for (var t, a = e.target, o = s.shapeList.length - 1; o >= 0; o--) t = s.type == i.CHART_TYPE_PIE || s.type == i.CHART_TYPE_FUNNEL ? n.get(s.shapeList[o], "name") : (n.get(s.shapeList[o], "series") || {}).name, t != a || s.shapeList[o].invisible || s.zr.addHoverShape(s.shapeList[o])
	...
	return t.prototype = {
				...
				for (var r = 0, s = t[a].length; s > r; r++) {
					...
					if ("-1" != i.indexOf("x")) {
						...
						e[l]["averageLine" + h] = [
							...
							[e[l]["minX" + h], this.component.grid.getYend()],
							[e[l]["minX" + h], this.component.grid.getY()]
						], e[l]["maxLine" + h] = [
							[e[l]["maxX" + h], this.component.grid.getYend()],
							[e[l]["maxX" + h], this.component.grid.getY()]
						], e[l].isHorizontal = !1
					}
					if (h = "xy" == i ? 1 : "", "-1" != i.indexOf("y")) {
						...
						e[l]["averageLine" + h] = [
							[this.component.grid.getX(), m],
							[this.component.grid.getXend(), m]
						], e[l]["minLine" + h] = [
							[this.component.grid.getX(), e[l]["minY" + h]],
							[this.component.grid.getXend(), e[l]["minY" + h]]
						], e[l]["maxLine" + h] = [
							[this.component.grid.getX(), e[l]["maxY" + h]],
							[this.component.grid.getXend(), e[l]["maxY" + h]]
						], e[l].isHorizontal = !0
					}
				}
		},
		...
		_buildMarkPoint: function(e) {
			...
			if (this.type === r.CHART_TYPE_FORCE || this.type === r.CHART_TYPE_CHORD)
				for (var l = 0, h = d.length; h > l; l++) this.zr.addShape(d[l])
		},
		...
		_markPoint: function(e, t) {
			...
			var a, o, l, h, d, c, p, u = [],
				...
				g = this.zr.getWidth(),
				f = this.zr.getHeight();
			if (t.large) a = this.getLargeMarkPoingShape(e, t), a._mark = "largePoint", a && u.push(a);
			else
				for (var b = 0, _ = V.length; _ > b; b++) null != V[b].x && null != V[b].y && (l = null != V[b] && null != V[b].value ? V[b].value : "", y && (o = y.getColor(i.name)), U && (o = isNaN(l) ? o : U.getColor(l), h = [V[b], t], d = this.deepQuery(h, "itemStyle.normal.color") || o, c = this.deepQuery(h, "itemStyle.emphasis.color") || d, null == d && null == c) || (o = null == o ? this.zr.getColor(e) : o, V[b].tooltip = V[b].tooltip || t.tooltip || {
					trigger: "item"
				}, V[b].name = null != V[b].name ? V[b].name : "", V[b].value = l, a = this.getSymbolShape(t, e, V[b], b, V[b].name, this.parsePercent(V[b].x, g), this.parsePercent(V[b].y, f), "pin", o, "rgba(0,0,0,0)", "horizontal"), a._mark = "point", p = this.deepMerge([V[b], t], "effect"), p.show && (a.effect = p), i.type === r.CHART_TYPE_MAP && (a._geo = this.getMarkGeo(V[b])), s.pack(a, i, e, V[b], b, V[b].name, l), u.push(a)));
			return u
		},
		...
		getLineMarkShape: function(e, t, i, n, o, r, s, l, h) {
			var d = null != i[0] ? null != i[0].value ? i[0].value : i[0] : "-",
				m = null != i[1] ? null != i[1].value ? i[1].value : i[1] : "-",
				c = [this.query(i[0], "symbol") || e.symbol[0], this.query(i[1], "symbol") || e.symbol[1]],
				p = [this.query(i[0], "symbolSize") || e.symbolSize[0], this.query(i[1], "symbolSize") || e.symbolSize[1]];
			p[0] = "function" == typeof p[0] ? p[0](d) : p[0], p[1] = "function" == typeof p[1] ? p[1](m) : p[1];
			var u = [this.query(i[0], "symbolRotate") || e.symbolRotate[0], this.query(i[1], "symbolRotate") || e.symbolRotate[1]],
				V = [i[0], e],
		...
		getLargeMarkPoingShape: function(e, t) {
			var i, n, a, r, s, l, h = this.series[e],
				d = this.component,
				m = t.data,
				c = d.dataRange,
				p = d.legend,
		...
		addShapeList: function() {
			var e, t, i = this.option.animationThreshold / (this.canvasSupported ? 2 : 4),
				...
				a = this.shapeList,
				o = n.length > 0 ? 500 : this.query(this.option, "animationDuration"),
		...
		animationEffect: function(e) {
			...
			if (null != t) {
				...
				this.canvasSupported && this.zr.modLayer(i, {
					motionBlur: !0,
					lastFrameAlpha: .95
				});
		...
		clearEffectShape: function(e) {
			this.zr && this.effectList && this.effectList.length > 0 && (e && this.zr.modLayer(r.EFFECT_ZLEVEL, {
				motionBlur: !1
		...
		delMark: function(e, t, i) {
			...
			if (this.selectedMap[n.name]) {
				for (var a = !1, o = [this.shapeList, this.effectList], r = 2; r--;)
					for (var l = 0, h = o[r].length; h > l; l++)
						if (o[r][l]._mark == i && s.get(o[r][l], "seriesIndex") == e && s.get(o[r][l], "name") == t) {
	...
	var i = e("zrender/shape/Base"),
		...
		h = e("zrender/tool/area"),
		d = e("zrender/shape/util/dashedLineTo"),
		m = e("zrender/shape/util/smoothSpline"),
	...
	return t.prototype = {
			...
			if (t.lineType && "solid" != t.lineType) {
				if ("dashed" == t.lineType || "dotted" == t.lineType)
					if ("spline" !== t.smooth) {
						...
						for (var r = 1; a > r; r += 2) e.moveTo(n[r - 1][0], n[r - 1][1]), e.lineTo(n[r][0], n[r][1])
			} else {
				e.moveTo(n[0][0], n[0][1]);
		...
		brushSymbol: function(e, t, i) {
			if ("none" != t.symbol[i]) {
				...
				var a, o = Math.min(t.pointList.length, Math.round(t.pointListLength || t.pointList.length)),
					r = 0 === i ? t.pointList[0][0] : t.pointList[o - 1][0],
					s = 0 === i ? t.pointList[0][1] : t.pointList[o - 1][1],
					h = "undefined" != typeof t.symbolRotate[i] ? t.symbolRotate[i] - 0 : 0;
				if (0 !== h && (a = l.create(), l.identity(a), (r || s) && l.translate(a, a, [-r, -s]), l.rotate(a, a, h * Math.PI / 180), (r || s) && l.translate(a, a, [r, s]), e.transform.apply(e, a)), "arrow" == t.iconType && 0 === h) this.buildArrawPath(e, t, i);
				else {
		...
		buildArrawPath: function(e, t, i) {
			var n = Math.min(t.pointList.length, Math.round(t.pointListLength || t.pointList.length)),
				a = 2 * t.symbolSize[i],
				o = t.pointList[0][0],
				r = t.pointList[n - 1][0],
				s = t.pointList[0][1],
			...
			var m = Math.PI / 8,
				...
				p = 0 === i ? s : l,
				u = [
					[c + a * Math.cos(d - m), p - a * Math.sin(d - m)],
					[c + .6 * a * Math.cos(d), p - .6 * a * Math.sin(d)],
					[c + a * Math.cos(d + m), p - a * Math.sin(d + m)]
				];
			e.moveTo(c, p);
		...
		getOffetPoint: function(e, t) {
			var i, n = Math.sqrt(Math.round((e[0] - t[0]) * (e[0] - t[0]) + (e[1] - t[1]) * (e[1] - t[1]))) / 3,
				a = [e[0], e[1]],
				o = .2;
			if (e[0] != t[0] && e[1] != t[1]) {
		...
		buildPath: function(e, t) {
			...
			if (0 !== n)
				for (var a, o, r, s, l, h = 1e4, d = Math.ceil(n / h), m = i[0] instanceof Array, c = t.size ? t.size : 2, p = c, u = c / 2, V = 2 * Math.PI, U = 0; d > U; U++) {
					...
					for (var y = a; o > y; y++)
						if (t.random && (r = t["randomMap" + y % 20] / 100, p = c * r * r, u = p / 2), m ? (s = i[y][0], l = i[y][1]) : (s = i[y].x, l = i[y].y), 3 > p) e.rect(s - u, l - u, p, p);
	...
	function t(e, t, i, n, a) {
		...
		o = t.style.pointList;
		var m = o.length;
		i.style.pointList = m == s ? o : s > m ? o.concat(r.slice(m)) : o.slice(0, s), e.addShape(i), e.animate(i.id, "style").when(n, {
			pointList: r
		}).start(a)
	}
	...
	function n(e, t, n, a, o) {
		...
		i(n, t, "x", "y", "width", "height"), n.position = t.position, e.addShape(n), (m[0] != t.position[0] || m[1] != t.position[1]) && e.animate(n.id, "").when(a, {
			position: m
		}).start(o), e.animate(n.id, "style").when(a, {
			x: s,
			y: l,
			width: h,
			height: d
	...
	function a(e, t, i, n, a) {
		...
		var r = i.style.y;
		i.style.y = t.style.y, e.addShape(i), e.animate(i.id, "style").when(n, {
			y: r
		}).start(a)
	}
	...
	function s(e, t, n, a, o) {
		...
		var r = n.style.x,
			s = n.style.y;
		i(n, t, "x", "y"), e.addShape(n), e.animate(n.id, "style").when(a, {
			x: r,
			y: s
		}).start(o)
	}
	...
	function l(t, i, n, a, o) {
		var r = e("zrender/shape/Polygon").prototype.getRect(n.style),
			s = r.x + r.width / 2,
			l = r.y + r.height / 2;
		n.scale = [.1, .1, s, l], t.addShape(n), t.animate(n.id, "").when(a, {
			scale: [1, 1, s, l]
		}).start(o)
	}
	...
	function d(e, t, i, n, a) {
		...
		var o = i.style.angle;
		i.style.angle = t.style.angle, e.addShape(i), e.animate(i.id, "style").when(n, {
			angle: o
		}).start(a)
	}
	...
	function m(e, t, i, a, o, r) {
		if (i.style._x = i.style.x, i.style._y = i.style.y, i.style._width = i.style.width, i.style._height = i.style.height, t) n(e, t, i, a, o);
			...
			var s = i._x || 0,
				l = i._y || 0;
			i.scale = [.01, .01, s, l], e.addShape(i), e.animate(i.id, "").delay(r).when(a, {
				scale: [1, 1, s, l]
			}).start(o || "QuinticOut")
		}
	}
	...
	function p(e, t, i, n, a) {
		i.style.smooth ? t ? e.addShape(i) : (i.style.pointListLength = 1, e.addShape(i), i.style.pointList = i.style.pointList || i.getPointList(i.style), e.animate(i.id, "style").when(n, {
			pointListLength: i.style.pointList.length
		}).start(a || "QuinticOut")) : (i.style.pointList = t ? t.style.pointList : [
			[i.style.xStart, i.style.yStart],
			[i.style.xStart, i.style.yStart]
		], e.addShape(i), e.animate(i.id, "style").when(n, {
			pointList: [
				[i.style.xStart, i.style.yStart],
				[i._x || 0, i._y || 0]
			]
	...
	function t(e, t, i, n) {
		var o = i.effect,
			l = o.color || i.style.strokeColor || i.style.color,
			d = o.shadowColor || l,
			m = o.scaleSize,
			c = "undefined" != typeof o.shadowBlur ? o.shadowBlur : m,
			p = new s({
				zlevel: n,
				style: {
					brushType: "stroke",
		...
		var u = window.devicePixelRatio || 1,
			V = (p.style.width / u - i.style._width) / 2;
		p.style.x = i.style._x - V, p.style.y = i.style._y - V;
		var U = 100 * (o.period + 10 * Math.random());
		e.modShape(i.id, {
			invisible: !0
		});
		var y = p.style.x + p.style.width / 2 / u,
			g = p.style.y + p.style.height / 2 / u;
		e.modShape(p.id, {
			scale: [.1, .1, y, g]
		}), e.animate(p.id, "", o.loop).when(U, {
			scale: [1, 1, y, g]
		}).done(function() {
	...
	function i(e, t, i, n) {
		var a = i.effect,
			...
			r = a.scaleSize,
			s = a.shadowColor || o,
			h = "undefined" != typeof a.shadowBlur ? a.shadowBlur : 2 * r,
			d = window.devicePixelRatio || 1,
			m = new l({
				zlevel: n,
				position: i.position,
		...
		t.push(m), e.addShape(m), e.modShape(i.id, {
			invisible: !0
		});
	...
	function n(e, t, i, n) {
		var s, l = i.effect,
			d = l.color || i.style.strokeColor || i.style.color,
			m = l.shadowColor || i.style.strokeColor || d,
			c = i.style.lineWidth * l.scaleSize,
			p = "undefined" != typeof l.shadowBlur ? l.shadowBlur : c,
			u = new o({
				zlevel: n,
				style: {
					x: p,
		...
		var V = (i.style.xStart - i.style.xEnd) * (i.style.xStart - i.style.xEnd) + (i.style.yStart - i.style.yEnd) * (i.style.yStart - i.style.yEnd),
			U = Math.round(Math.sqrt(Math.round(V * l.period * l.period)));
		if (i.style.smooth) {
			var y = i.style.pointList || i.getPointList(i.style),
				g = y.length;
			U = Math.round(U / g);
		...
		} else e.animate(u.id, "style", l.loop).when(U, {
			x: i._x - s,
			y: i._y - s
		}).done(function() {
			i.effect.show = !1, e.delShape(u.id)
	...
	var a = e("../util/ecData"),
		...
		h = e("zrender/tool/env").canvasSupported;
	return {
		point: t,
		largePoint: i,
		line: n
		...
		_contentToOption: function(e) {
			...
			for (t = 0, o = m.length; o > t; t++)
				if (d++, m[t].type == n.CHART_TYPE_SCATTER)
					for (var i = 0, a = m[t].data.length; a > i; i++) s = e[d], l = s.replace(" ", "").split(","), "undefined" != typeof m[t].data[i].value ? m[t].data[i].value = l : m[t].data[i] = l, d++;
				else {
					s = e[d].split(",");
	...
	return t.prototype = {
		...
		_getDetail: function() {
			var e = "horizontal" == this.zoomOption.orient ? "xAxis" : "yAxis",
			...
			for (var i in t) {
				...
				if (null != n) {
					var a = n.length,
						...
						r = Math.ceil(this._zoom.end / 100 * a);
					return r -= r >= a ? 1 : 0, {
			...
			var l = this._zoom.seriesIndex[0],
				...
				d = this.option[e][h].type,
				m = this._zoom.scatterMap[l][e.charAt(0)].min,
				c = this._zoom.scatterMap[l][e.charAt(0)].max,
				p = c - m;
			if ("value" == d) return {
				start: m + p * this._zoom.start / 100,
				end: m + p * this._zoom.end / 100
			};
			if ("time" == d) {
	...
	return t.prototype = {
		...
		_getReformedLabel: function(e) {
			var t = "undefined" != typeof this.option.data[e].value ? this.option.data[e].value : this.option.data[e],
		...
		_buildAxisTick: function() {
			var e, t = this.option.data.length,
				...
				o = i.lineStyle.color,
				r = i.lineStyle.width,
				s = "auto" == i.interval ? this._interval : i.interval - 0 + 1,
				l = i.onGap,
				h = l ? this.getGap() / 2 : "undefined" == typeof l && this.option.boundaryGap ? this.getGap() / 2 : 0,
				d = h > 0 ? -s : 0;
			if (this.isHorizontal())
				for (var m, c = "bottom" == this.option.position ? i.inside ? this.grid.getYend() - n - 1 : this.grid.getYend() + 1 : i.inside ? this.grid.getY() + 1 : this.grid.getY() - n - 1, p = d; t > p; p += s) m = this.subPixelOptimize(this.getCoordByIndex(p) + (p >= 0 ? h : 0), r), e = {
					_axisShape: "axisTick",
					zlevel: this._zlevelBase,
					hoverable: !1,
					style: {
				...
				for (var u, V = "left" == this.option.position ? i.inside ? this.grid.getX() + 1 : this.grid.getX() - n - 1 : i.inside ? this.grid.getXend() - n - 1 : this.grid.getXend() + 1, p = d; t > p; p += s) u = this.subPixelOptimize(this.getCoordByIndex(p) - (p >= 0 ? h : 0), r), e = {
					_axisShape: "axisTick",
					zlevel: this._zlevelBase,
					hoverable: !1,
					style: {
		...
		_buildAxisLabel: function() {
			var e, t, i = this.option.data,
				...
				l = this.option.axisLabel.clickable,
				h = this.option.axisLabel.textStyle;
			if (this.isHorizontal()) {
				var d, m;
		...
		_buildSplitLine: function() {
			...
			var s = r.length,
				l = i.onGap,
				h = l ? this.getGap() / 2 : "undefined" == typeof l && this.option.boundaryGap ? this.getGap() / 2 : 0;
			if (t -= l || "undefined" == typeof l && this.option.boundaryGap ? 1 : 0, this.isHorizontal())
				for (var d, m = this.grid.getY(), c = this.grid.getYend(), p = 0; t > p; p += this._interval) d = this.subPixelOptimize(this.getCoordByIndex(p) + h, o), e = {
					zlevel: this._zlevelBase,
					hoverable: !1,
					style: {
						xStart: d,
				...
				for (var u, V = this.grid.getX(), U = this.grid.getXend(), p = 0; t > p; p += this._interval) u = this.subPixelOptimize(this.getCoordByIndex(p) - h, o), e = {
					zlevel: this._zlevelBase,
					hoverable: !1,
					style: {
						xStart: V,
		...
		_buildSplitArea: function() {
			...
			if (i instanceof Array) {
				var n = i.length,
					a = this.option.data.length,
					r = t.onGap,
					s = r ? this.getGap() / 2 : "undefined" == typeof r && this.option.boundaryGap ? this.getGap() / 2 : 0;
				if (this.isHorizontal())
					for (var l, h = this.grid.getY(), d = this.grid.getHeight(), m = this.grid.getX(), c = 0; a >= c; c += this._interval) l = a > c ? this.getCoordByIndex(c) + s : this.grid.getXend(), e = {
						zlevel: this._zlevelBase,
						hoverable: !1,
						style: {
							x: m,
							...
							width: l - m,
							height: d,
							color: i[c / this._interval % n]
						}
					}, this.shapeList.push(new o(e)), m = l;
				else
					for (var p, u = this.grid.getX(), V = this.grid.getWidth(), U = this.grid.getYend(), c = 0; a >= c; c += this._interval) p = a > c ? this.getCoordByIndex(c) - s : this.grid.getY(), e = {
						zlevel: this._zlevelBase,
						hoverable: !1,
						style: {
							x: u,
		...
		getIndexByName: function(e) {
			for (var t = this.option.data, i = t.length, n = 0; i > n; n++)
				if (t[n] == e || "undefined" != typeof t[n].value && t[n].value == e) return n;
			return -1
		},
	...
	return t.prototype = {
		...
		_buildAxisTick: function() {
			...
			if (this.isHorizontal())
				...
				for (var m, c = "left" === this.option.position ? n.inside ? this.grid.getX() + 1 : this.grid.getX() - o - 1 : n.inside ? this.grid.getXend() - o - 1 : this.grid.getXend() + 1, d = 0; i > d; d++) m = this.subPixelOptimize(this.getCoord(t[d]), s), e = {
					_axisShape: "axisTick",
					zlevel: this._zlevelBase,
					hoverable: !1,
					style: {
		...
		_buildSplitLine: function() {
			...
			var l = s.length;
			if (this.isHorizontal())
				for (var h, d = this.grid.getY(), m = this.grid.getYend(), c = 0; i > c; c++) h = this.subPixelOptimize(this.getCoord(t[c]), r), e = {
					zlevel: this._zlevelBase,
					hoverable: !1,
					style: {
						xStart: h,
				...
				for (var p, u = this.grid.getX(), V = this.grid.getXend(), c = 0; i > c; c++) p = this.subPixelOptimize(this.getCoord(t[c]), r), e = {
					zlevel: this._zlevelBase,
					hoverable: !1,
					style: {
						xStart: u,
		...
		_buildSplitArea: function() {
			...
			if (t instanceof Array) {
				...
				if (this.isHorizontal())
					...
					for (var m, c = this.grid.getX(), p = this.grid.getWidth(), u = this.grid.getYend(), d = 0; a >= d; d++) m = a > d ? this.getCoord(n[d]) : this.grid.getY(), e = {
						zlevel: this._zlevelBase,
						hoverable: !1,
						style: {
							x: c,
		...
		_calculSum: function(e, t) {
			...
			if (this.series[t].stack) {
				...
				for (var h = 0, d = n.length; d > h; h++)
					for (var m = n[h].evolution, c = 0, p = m.length; p > c; c++) e[a].push(s.getNewDate(m[c].time))
			}
		},
		...
		_reformLabelData: function(e) {
			...
			if (t)
				...
				for (var i = 0, n = this._valueList.length; n > i; i++) this._valueLabel.push(s.format(e, this._valueList[i]));
			else
				for (var i = 0, n = this._valueList.length; n > i; i++) this._valueLabel.push(this.numAddCommas(this._valueList[i]))
		},
